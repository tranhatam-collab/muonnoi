...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  await env.D1.prepare(
    `UPDATE investors SET investor_status = 'approved', approved_by = ?, approved_at = datetime('now'), updated_at = datetime('now')
     WHERE id = ?`
  ).bind(admin.userId, investorId).run();

  await env.D1.prepare(
    `INSERT INTO audit_logs (id, actor_id, actor_role, action, target_type, target_id, created_at)
     VALUES (?, ?, 'admin', 'investor_approved', 'investor', ?, datetime('now'))`
  ).bind(`audit_${Date.now()}`, admin.userId, investorId).run();

  return json({ ok: true, message: 'Investor approved' }, 200, origin);
};
