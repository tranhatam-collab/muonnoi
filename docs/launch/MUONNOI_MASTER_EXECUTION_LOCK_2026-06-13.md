...
> **Trạng thái:** LOCKED FOR DEV.
> **ĐÍNH CHÍNH 2026-06-13 (sau khi đọc HẾT 15 file canonical + 3 file homepage/menu):** sửa 2 lỗi do đọc thiếu — (1) chochoi=Play PORTAL, plays=module con (không đổi tên); (2) nhachung=Living, ai=AI layer (2 surface riêng). SSO/dependency là canonical đã khóa, không phải đề xuất. Xem Mục 9.
 Không được tự ý đổi tổng kế hoạch. Mọi thay đổi lớn phải Founder duyệt.
...
| nhachung.muonnoi.org | **Muốn Sống / Living Ecosystem** (trụ, ≠ ai) | **000** | ❌ CHẾT. Canonical = Living (nodes vật lý, real-life loop). KHÁC `ai` — không gộp. |
| chochoi.muonnoi.org | **Muốn Chơi / Play PORTAL** (trụ) | **000** | ❌ CHẾT. Canonical = **portal cha** chứa plays/trochoi/sukien/giaitri/game/challenge/festival. KHÔNG phải alias của plays. |
| plays.muonnoi.org | Online games (module CON của chochoi) | 200 | ✅ 100 game live — đúng vai trò con. Cần chochoi portal đứng trên. |
...
| ai.muonnoi.org | **AI Assistance Layer** (≠ nhachung) | 200 | ✅ Live. Canonical 07 = AI layer xuyên hệ. KHÔNG gộp với nhachung (Living). Lưu ý: title "Nhà Chung" trên ai là nhầm nội dung — cần tách. |
...
|---|---------|------|-----------|
| 4 | **chochoi.muonnoi.org** | Build **Play PORTAL** (cha) đứng trên plays (con). plays GIỮ nguyên là online-games module; thêm trochoi/sukien khi tới. (Canonical 08 CHOCHOI: portal, không alias) | `curl chochoi.muonnoi.org` = 200, link xuống plays |
| 5 | **nhachung.muonnoi.org** | Build **Living Ecosystem** (nodes vật lý, real-life loop) — surface RIÊNG, không gộp với `ai` (AI layer). (Canonical 11 NHACHUNG) | `curl nhachung.muonnoi.org` = 200 |
...
## 9. ĐÃ KHÓA TRONG CANONICAL (KHÔNG hỏi lại) + QUYẾT ĐỊNH THẬT SỰ CÒN MỞ

**Đã khóa canonical — tao từng hỏi nhầm, nay đính chính (không phải open question):**
- ✅ `chochoi` = Play PORTAL; `plays/trochoi/sukien/giaitri/game/challenge/festival` = modules con (08 CHOCHOI + 03 ECOSYSTEM). KHÔNG đổi tên plays.
- ✅ `nhachung` = Living Ecosystem; `ai` = AI Assistance Layer — 2 surface RIÊNG (03 + 07 + 11). KHÔNG gộp.
- ✅ SSO/“cùng 1 profile, 1 trust state, 1 timeline” = **đã khóa** trong 02 ARCHITECTURE + 03 ECOSYSTEM (dependency rule). Không phải đề xuất của tao — là canonical. → Làm, không cần hỏi.
- ✅ verify = Public Trust Infra (moat); truyenthong = Media infra; dautu = member layer — tất cả đã khóa.

**Quyết định THẬT SỰ còn mở (chặn việc):**
1. **Thứ tự build 4 trụ/infra đang 000** (verify, chochoi-portal, nhachung, truyenthong): cái nào trước? (Tao đề xuất verify trước vì là moat + chặn trust loop.)
2. **truyenthong:** build trong sprint này hay ghi PLANNED ở /ecosystem (readiness badge) và làm sau?
3. **lamviec/aiesec:** fix A hay B (Mục 5)? App "Làm Việc" Next.js cũ giữ hay thay? (đây là blocker đang mở thật)
4. **dulich regression:** rebind ngay (đang 000) hay để PLANNED?
5. **Nguồn lực:** SSO spine + verify layer là 2 việc nặng nhất — làm song song hay tuần tự?

→ Chốt 5 điểm này, team chạy theo Mục 7. Phần đã khóa canonical thì làm thẳng, không hỏi lại.
...
## 10. ÁP DỤNG CHUẨN WPAS + BRANDPRO v2.0 (NEW 2026-06-13)

**Mục tiêu:** Áp dụng 2 bộ chuẩn vận hành cho toàn team (muonnoi.org + các sub).

**Timeline:** 4 phase × 2 tuần = 8 tuần (có thể kéo dài 10 tuần).

**Quyết định Founder (PHẢI TRẢ LỜI TRƯỚC KHI BẮT ĐẦU):**
1. **Budget level:** Lean (< 200tr) / Standard (200–800tr) / Premium (> 800tr)
2. **Timeline:** 8 tuần / 10 tuần / 12 tuần
3. **Brand Guardian:** Bổ nhiệm ai?

**Phase 1 — WPAS Adoption (2 tuần):**
- Setup staging environment
- Setup QA infrastructure (checklist, severity matrix, evidence matrix)
- Setup production gate
- Deploy thử theo quy trình 12 bước

**Phase 2 — Brandpro Foundation (4 tuần):**
- Brand Forensics + Risk Matrix
- Naming Strategy + Brand Pillars
- Verbal Identity + Visual Identity + Brand Book
- Bilingual Messaging + SEO Entity

**Phase 3 — Governance & Trackers (2 tuần):**
- Bổ nhiệm Brand Guardian
- Setup Asset Hub
- Setup Approval Workflow
- Setup Trackers (Budget, Risk, Content, Asset Version, KPI, Product Brand)

**Phase 4 — Tech Product Branding (2 tuần):**
- Product Brand System (feature naming, in-app copy, onboarding branding)
- Design System Dev Handoff (design tokens, components, Figma-to-code)

**Chi tiết:** Xem `STANDARD_ADOPTION_PLAN_2026-06-13.md`

**Templates:**
- WPAS: `WPAS/` (18 file)
- Brandpro: `BRANDPRO/` (20 file) + `BRANDPRO/_artifacts/` (22 artefact)

**Audit:** Xem `STANDARD_ADOPTION_AUDIT_2026-06-13.md`

**Authority:**
- WPAS: Quy trình production acceptance bắt buộc
- Brandpro: Brand strategy foundation bắt buộc
- Mọi deploy phải qua WPAS
- Mọi brand work phải qua Brandpro
